...
type KeyType<KeysetsMap extends Record<string, LanguageConfig>> = keyof KeysetType<KeysetsMap>;

type KeysetType<KeysetsMap extends Record<string, LanguageConfig>> = UnwrapKeysetType<
  KeysetsMap[keyof KeysetsMap]['keyset']
>;

type UnwrapKeysetType<MaybeUnresolvedKeyset extends Keyset | (() => Promise<Keyset>)> =
  MaybeUnresolvedKeyset extends () => Promise<infer ResolvedKeyset> ? ResolvedKeyset : MaybeUnresolvedKeyset;
...
export class I18N<KeysetsMap extends Record<string, LanguageConfig>> {
  ...
  get<Key extends KeyType<KeysetsMap>>(key: Key, ...rest: GetRestParams<KeysetsMap, Key>): string {
    ...
    const translation: string | Record<string, string> | undefined = keyset[key];
  ...
  subscribe(cb: (fn: keyof KeysetsMap) => void, options?: {immediate: boolean}) {
...
function interpolateTranslation(translation: string, params: Record<string, string | number>) {
